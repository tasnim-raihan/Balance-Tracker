...
object LedgerCalculator {
    ...
    data class CalculationResult(
        ...
        val totalEarningsBreakdown: Int = 0
    ) {
        val isSurplus: Boolean get() = deficit < 0
        val totalVariance: Int get() = if (isSurplus) abs(deficit) else deficit
        
        // Unified position calculation where positive values are Income, negative values are Loss
        val netValue: Int get() {
            val signedVariance = if (isSurplus) totalVariance else -totalVariance
            return signedVariance - declaredDeficit + totalEarningsBreakdown
        }
        
        val unexplainedRemainingProfit: Int get() = if (netValue > 0) netValue else 0
        val unexplainedRemainingLoss: Int get() = if (netValue < 0) abs(netValue) else 0
    }
    ...
    /**
         ...
         * based on the refined algorithms using signed balance checks.
         */
    fun analyzeVariance(
        expectedBalance: Int,
        walletBalance: Int,
        ...
        val totalVariance = abs(deficit)
        
        val signedVariance = if (isSurplus) totalVariance else -totalVariance
        val netValue = signedVariance - totalSpendingBreakdown + totalEarningsBreakdown
        
        val unexplainedRemainingProfit = if (netValue > 0) netValue else 0
        val unexplainedRemainingLoss = if (netValue < 0) abs(netValue) else 0
    ...
    fun calculate(
        ...
        // Step 5: Unified ledger remaining calculation using formula:
        val isSurplus = deficit < 0
        val signedVariance = if (isSurplus) abs(deficit) else -abs(deficit)
        val netValue = signedVariance - declaredDeficit + totalEarningsBreakdown
        val ledgerLoss = if (netValue < 0) abs(netValue) else 0
        ...
        return CalculationResult(
            ...
            totalEarningsBreakdown = totalEarningsBreakdown
        )
    }
}
